...
{
    public class IniFileHM
    {
        private readonly string iniFile = "";
        public IniFileHM(string Path)
        { this.iniFile = Path; }
        ...
        public string INIGetStringValue(string section, string key, string defaultValue = "")
        {
            ...
            {
                throw new ArgumentException("必须指定键名称(key)", nameof(key));
            }
        ...
        /// <param name="defaultValue">如果没此KEY所使用的默认值</param>  
        /// <returns>读取到的值</returns>  
        public int INIGetStringValue(string section, string key, int defaultValue = 0)
        {
            string str = INIGetStringValue(section, key, defaultValue.ToString());
            try
            {
            ...
            {
                Console.WriteLine("1036:" + ex.Message);
        ...
        /// <param name="defaultValue">如果没此KEY所使用的默认值</param>  
        /// <returns>读取到的值</returns>  
        public double INIGetStringValue(string section, string key, double defaultValue = 0)
        {
            string str = INIGetStringValue(section, key, defaultValue.ToString());
            try
            {
            ...
            {
                Console.WriteLine("1037:" + ex.Message);
                INIWriteValue(section, key, defaultValue.ToString());
                return defaultValue;
            }
        }
        ...
        /// <param name="defaultValue">如果没此KEY所使用的默认值</param>  
        /// <returns>读取到的值</returns>  
        public bool INIGetStringValue(string section, string key, bool defaultValue = false)
        {
            string str = INIGetStringValue(section, key, defaultValue.ToString());
            try
            {
            ...
            {
                Console.WriteLine("1038:" + ex.Message);
                INIWriteValue(section, key, defaultValue.ToString());
                return defaultValue;
            }
        }
        ...
        /// <returns>操作是否成功</returns>  
        public bool INIWriteValue(string section, string key, object value)
        {
            if (string.IsNullOrEmpty(section))
            {
                throw new ArgumentException("必须指定节点名称", nameof(section));
            ...
            else
            {
                ...
                return WritePrivateProfileString(section, key, strValue, iniFile);
            }
        }
